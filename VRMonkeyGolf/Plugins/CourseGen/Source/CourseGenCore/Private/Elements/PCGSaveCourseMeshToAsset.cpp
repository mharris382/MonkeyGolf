...
#include "Elements/PCGSaveCourseMeshToAsset.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSaveCourseMeshToAsset)

#define LOCTEXT_NAMESPACE "PCGSaveCourseMeshToAssetElement"

#if WITH_EDITOR
FName UPCGSaveCourseMeshToAssetSettings::GetDefaultNodeName() const
{
	return FName(TEXT("SaveDynamicMeshToAsset"));
}

FText UPCGSaveCourseMeshToAssetSettings::GetDefaultNodeTitle() const
{
	return LOCTEXT("NodeTitle", "Save Dynamic Mesh To Asset");
}

FText UPCGSaveCourseMeshToAssetSettings::GetNodeTooltipText() const
{
	return LOCTEXT("NodeTooltip", "Saves dynamic mesh data into a static mesh asset.");
}
#endif // WITH_EDITOR

FPCGElementPtr UPCGSaveCourseMeshToAssetSettings::CreateElement() const
{
	return MakeShared<FPCGSaveCourseMeshToAssetElement>();
}

TArray<FPCGPinProperties> UPCGSaveCourseMeshToAssetSettings::InputPinProperties() const
{
...
TArray<FPCGPinProperties> UPCGSaveCourseMeshToAssetSettings::OutputPinProperties() const
{ 
	TArray<FPCGPinProperties> Properties;
...
bool FPCGSaveCourseMeshToAssetElement::ExecuteInternal(FPCGContext* InContext) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGSaveCourseMeshToAssetElement::Execute);

	check(InContext);

	// For now we only support writing to an asset in editor build.
#if WITH_EDITOR
	const UPCGSaveCourseMeshToAssetSettings* Settings = InContext->GetInputSettings<UPCGSaveCourseMeshToAssetSettings>();
	check(Settings);
